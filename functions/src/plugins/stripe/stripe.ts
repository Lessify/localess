...
import { findPluginById } from '../../services';
import { canPerform } from '../../utils/security-utils';
import { ContentDocument, ContentKind, Plugin, PluginActionData, UserPermission } from '../../models';
...
expressApp.post('/api/stripe/2024-06-20/spaces/:spaceId/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
  ...
  const stripe = new Stripe(apiSecretKey, { apiVersion: '2025-02-24.acacia' });
