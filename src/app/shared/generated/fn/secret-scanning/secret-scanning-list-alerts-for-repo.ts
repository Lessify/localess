...
export interface SecretScanningListAlertsForRepo$Params {
...
export function secretScanningListAlertsForRepo(http: HttpClient, rootUrl: string, params: SecretScanningListAlertsForRepo$Params, context?: HttpContext): Observable<StrictHttpResponse<Array<SecretScanningAlert>>> {
  const rb = new RequestBuilder(rootUrl, secretScanningListAlertsForRepo.PATH, 'get');
  if (params) {
  ...
  return http.request(
    ...
    map((r: HttpResponse<any>) => {
      return r as StrictHttpResponse<Array<SecretScanningAlert>>;
    })
  );
}

secretScanningListAlertsForRepo.PATH = '/repos/{owner}/{repo}/secret-scanning/alerts';
