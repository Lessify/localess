...
export interface OrgsListPatGrants$Params {
...
export function orgsListPatGrants(http: HttpClient, rootUrl: string, params: OrgsListPatGrants$Params, context?: HttpContext): Observable<StrictHttpResponse<Array<OrganizationProgrammaticAccessGrant>>> {
  const rb = new RequestBuilder(rootUrl, orgsListPatGrants.PATH, 'get');
  if (params) {
  ...
  return http.request(
    ...
    map((r: HttpResponse<any>) => {
      return r as StrictHttpResponse<Array<OrganizationProgrammaticAccessGrant>>;
    })
  );
}

orgsListPatGrants.PATH = '/orgs/{org}/personal-access-tokens';
