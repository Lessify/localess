...
export class MeService {
  ...
  updateEmail(user: User, newEmail: string): Observable<void> {
    return from(updateEmail(user, newEmail)).pipe(traceUntilFirst('Firestore:Me:updateEmail'));
  }

  updatePassword(user: User, newPassword: string): Observable<void> {
    return from(updatePassword(user, newPassword)).pipe(traceUntilFirst('Firestore:Me:updatePassword'));
  }
}
