...
export class PluginService {
  ...
  findAll(spaceId: string): Observable<Plugin[]> {
    ...
    return collectionData(query(collection(this.firestore, `spaces/${spaceId}/plugins`), ...queryConstrains), { idField: 'id' }).pipe(
  ...
  findAllAvailable(spaceId: string): Observable<PluginDefinition[]> {
    return collectionData(collection(this.firestore, `spaces/${spaceId}/plugins`), { idField: 'id' }).pipe(
      traceUntilFirst('Firestore:Plugins:findAll'),
      map(it => it as Plugin[]),
      map(dbPlugins => {
        const pcl: PluginDefinition[] = [];
        Object.getOwnPropertyNames(AVAILABLE_PLUGINS_MAP).forEach(aPluginId => {
  ...
  sendEvent(spaceId: string, id: string, actionId: string): Observable<void> {
    ...
    return pluginEvent({ spaceId }).pipe(traceUntilFirst(`Functions:Plugins:sendEvent:${id}-${actionId}`));
  }
  ...
  uninstall(spaceId: string, id: string): Observable<void> {
    return from(deleteDoc(doc(this.firestore, `spaces/${spaceId}/plugins/${id}`))).pipe(traceUntilFirst('Firestore:Plugins:delete'));
  }
}
...
const AVAILABLE_PLUGINS_MAP: Record<string, PluginDefinition> = {
  ...
  //             description:
  //               'The unit amount in rappen/cent to be charged, represented as a whole integer if possible. Only set if billing_scheme=per_unit.',
  //             required: false,
  //             translatable: false,
  //           } as SchemaFieldNumber,
  //           {
