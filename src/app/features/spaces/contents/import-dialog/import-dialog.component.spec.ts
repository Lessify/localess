...
describe('ImportDialogComponent', () => {
  ...
  it('accepts a .llc.zip file', async () => {
    const { component } = setup();
    const file = new File(['data'], 'contents.llc.zip');

    await component.onFileChange(fileChangeEvent(file));

    expect(component.fileName).toBe('contents.llc.zip');
    expect(component.fileWrong).toBe(false);
    expect(component.form.value.file).toBe(file);
    expect(component.form.valid).toBe(true);
  });

  it('flags a file without the .llc.zip extension', async () => {
    const { component } = setup();
