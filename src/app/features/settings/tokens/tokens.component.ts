...
// Subscriptions
      ...
      .pipe(
        ...
        switchMap(it =>
          combineLatest([
            ...
            this.tokenService.findAll(it.id)
          ])
        ),
  ...
  openAddDialog(): void {
    if (this.selectedSpace) {
      ...
      this.dialog.open<TokenDialogComponent, never, TokenDialogModel>(
        ...
        .pipe(
          ...
          switchMap(it =>
            this.tokenService.create(spaceId, it?.name!)
          )
        )
        .subscribe({
            next: (value) => {
    ...
    if (this.selectedSpace) {
      ...
      this.dialog.open<ConfirmationDialogComponent, ConfirmationDialogModel, boolean>(
        ...
        .pipe(
          filter((it) => it || false),
          switchMap(_ =>
            this.tokenService.delete(spaceId, element.id)
          )
        )
        .subscribe({
          next: () => {
